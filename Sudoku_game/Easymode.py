...
def solve(bord):
    ...
    for num in range(1, 10):  # 1-9
        ...
        # lb = easysudoku.nametowidget(position)
...
def placeBoard():
    ...
    for i in range(len(sudoku)):
        ...
        if i % 3 == 0 and i != 0:
            display_lab = Label(easysudoku, text="  ", bg='gainsboro')
        ...
        for j in range(len(sudoku[i])):
            ...
            if j % 3 == 0 and j != 0:
                display_lab = Label(easysudoku, text="  ", bg='gainsboro')  # heller Abstand zwischen den 9x9 Feldern
            ...
            if num == 0:
                ...
                display_lab = Label(easysudoku, text="  ", bg='grey', name=name)  # for row and col

            else:
                display_lab = Label(easysudoku, text=num, bg='white', name=name)  # for row and col
...
# lb = Label(easysudoku, text='hello guys', name='guys')
# lb.grid(row=10, column=10)


# easysudoku.nametowidget(".0-9")
...
b2 = Button(easysudoku, text="+", command=plusNumber)
b3 = Button(easysudoku, text="-", command=minusNumber)
b2.grid(row=12, column=12)
b3.grid(row=13, column=12)

lb = Label(easysudoku, text=8, bg='grey', name="number")
lb.grid(row=12, column=13)

b4 = Button(easysudoku, text="Generate", command=generate_sudoku, bg='yellow')
b4.grid(row=12, column=14)

b = Button(easysudoku, text="Solve!", command=placeBoard, bg='green')
